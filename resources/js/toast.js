/**
 * Toast notifications: showToast({ type, message, duration }).
 * Requires #toast-container in DOM. data-toast-dismiss on close buttons.
 */
...
function showToast({ type = 'info', message = '', duration = TOAST_DURATION } = {}) {
    ...
    el.className = `flex items-start gap-3 rounded-xl border px-4 py-3 shadow-md pointer-events-auto ${variant} transition-opacity duration-300`;
    el.innerHTML = `
    ...
    const dismiss = () => {
        el.classList.add('opacity-0', 'translate-x-4', 'transition-all', 'duration-200');
    ...
    if (duration > 0) {
        el.setAttribute('data-toast-auto-dismiss', String(duration));
